...
/************************************************************************
*  class IEEE1394Boardv2::Camera						*
************************************************************************/
IEEE1394Boardv2::Camera::Camera(Ieee1394Port& port, int channel,
			      u_int64 uniqId)
    :Ieee1394Camera(port, channel, uniqId),
     _grayimage(), _grayimageIsReady(false),
     _rgbimage(), _rgbimageIsReady(false)
{
}

u_char
IEEE1394Boardv2::Camera::get_pixel(int col, int row)
{
    if (!_grayimageIsReady)
    {
	*this >> _grayimage;
	_grayimageIsReady = true;
    }
    return _grayimage[row][col];
}
	
RGB
IEEE1394Boardv2::Camera::get_rgbPixel(int col, int row)
{
    if (!_rgbimageIsReady)
    {
	*this >> _rgbimage;
	_rgbimageIsReady = true;
    }
    return _rgbimage[row][col];
}
	
/************************************************************************
*  class IEEE1394Boardv2							*
************************************************************************/
IEEE1394Boardv2::IEEE1394Boardv2(int board_id)
    :_port(board_id), _cameras()
{
    using namespace	std;
    
    string	configfile = string(VFGB_IEEE1394_CONFDIR)
			   + "/ieee1394board." + char('0' + board_id) + string (".color");
    ifstream	in(configfile.c_str());
    if (!in)
	throw std::invalid_argument("Cannot open config file for IEEE1394 board.");
    u_int	delay, ncameras;
    in >> delay >> ncameras;
    ODEBUG("Number of cameras: " << ncameras);
    _port.setDelay(delay);
    _cameras.resize(ncameras);
    ODEBUG("_cameras.size :" << _cameras.size());
    for (u_int i = 0; i < nchildren(); i++)
    {
	char	s[32];
	in >> s;
	u_int64	uniqId = strtoull(s, 0, 0);
	ODEBUG("CAMERA " << i << " GID: " << hex << uniqId << dec);
	_cameras[i] = new IEEE1394Boardv2::Camera(_port, i, uniqId);
	
	ODEBUG( " Format " << _cameras[i]->getFormat() << " Rate: " << _cameras[i]->getFrameRate());
...
IEEE1394Boardv2::~IEEE1394Boardv2()
{
    for (u_int i = 0 ; i <nchildren();  i++)
	delete _cameras[i];
}
...
void 
IEEE1394Boardv2::SetFeatureForCamera(u_int camera, Ieee1394Camera::Feature aFeature, u_int avalue)
{
  if ((camera>=0) && (camera<nchildren()))
    _cameras[camera]->setValue(aFeature,avalue);
}

void 
IEEE1394Boardv2::GetFeatureForCamera(u_int camera, Ieee1394Camera::Feature aFeature, u_int &avalue)
{
  if ((camera>=0) && (camera<nchildren()))
    avalue = _cameras[camera]->getValue(aFeature);
}


void
IEEE1394Boardv2::snap()
{
    for (u_int i= 0; i < nchildren(); i++)
      {
	_cameras[i]->snap();
...
IEEE1394Boardv2::snap(T* _lbuf)
{
    for (u_int i = 0; i < nchildren(); i++)
      {
	ODEBUG("BufferSize () = " << _cameras[i]->bufferSize());
	_cameras[i]->snap(_lbuf[i]);
      }
}


template <class T> void
IEEE1394Boardv2::snapfromI(T* _lbuf, unsigned int camera)
{
  if((camera>= 0) && (camera < nchildren()))
    {
      ODEBUG("BufferSize () = " << _cameras[camera]->bufferSize() << " for camera" << camera);
      //_cameras[camera]->doFlushListenBuffer();
      _cameras[camera]->snap(_lbuf);
      ODEBUG("Snap realized " << _cameras[camera]->bufferSize() << " for camera " << camera);
    }
}

template <class T> void
IEEE1394Boardv2::snapcfromI(T* _lbuf, unsigned int camera)
{
  if((camera>= 0) && (camera < nchildren()))
    {
      ODEBUG("BufferSize () = " << _cameras[camera]->bufferSize() << " for camera" << camera);
      //      _cameras[camera]->doFlushListenBuffer();
      _cameras[camera]->snapc(_lbuf);
      ODEBUG("Snap realized " << _cameras[camera]->bufferSize() << " for camera " << camera);
    }
}

template <class T> void
IEEE1394Boardv2::snapc(T* _lbuf)
{
    for (u_int i = 0; i < nchildren(); i++)
      {
	ODEBUG("BufferSize () = " << _cameras[i]->bufferSize());
	_cameras[i]->snapc(_lbuf[i]);
      }
}

void
IEEE1394Boardv2::continuousShot()
{
    for (u_int i = 0; i < nchildren(); i++)
...
void
IEEE1394Boardv2::stopContinuousShot()
{
    for (u_int i = 0; i < nchildren(); i++)
 ...
 * class Images Input                                         *
 **************************************************************/
HRP2IEEE1394ImagesInputMethod::HRP2IEEE1394ImagesInputMethod() : HRP2ImagesInputMethod()
{
  m_Board = 0;
  ...
  for(unsigned  li=0;li<m_Board->nchildren();li++)
  ...
  for(unsigned int i=0;i<m_Board->nchildren();i++)
...
void HRP2IEEE1394ImagesInputMethod::GetCameraFeatureValue(string aCamera, string aFeature, string &aValue)
{
  if (m_Board==0)
    return;
...
void HRP2IEEE1394ImagesInputMethod::SetCameraFeatureValue(string aCamera, string aFeature, string aValue)
{
  if (m_Board==0)
    return;
...
HRP2IEEE1394ImagesInputMethod::~HRP2IEEE1394ImagesInputMethod()
{
...
int HRP2IEEE1394ImagesInputMethod::StopProcess()
{
...
int HRP2IEEE1394ImagesInputMethod::GetSingleImage(unsigned char **Image, int camera, struct timeval &timestamp)
{
  int r=-1;
  if ((m_Computing==0) && (m_Board!=0))
    StopBoard();
  if ((m_Computing==1) && (m_Board==0))
    StartProcess();
...
int HRP2IEEE1394ImagesInputMethod::GetImage(unsigned char **ImageLeft, unsigned char **ImageRight, 
...
int HRP2IEEE1394ImagesInputMethod::GetImageRaw(unsigned char **ImageLeft, unsigned char **ImageRight, 
					       unsigned char **ImageUp, unsigned char **ImageWide)
{
...
int HRP2IEEE1394ImagesInputMethod::GetImagePGM(unsigned char **ImageLeft, unsigned char **ImageRight, 
					       unsigned char **ImageUp, unsigned char **ImageWide)
{
  unsigned char * ImagesDst[4];
...
int HRP2IEEE1394ImagesInputMethod::GetImageSinglePGM(unsigned char **Image, int camera, struct timeval &timestamp)
{
...
int HRP2IEEE1394ImagesInputMethod::GetImageSingleRGB(unsigned char **Image, int camera, struct timeval &timestamp)
{
  unsigned char * ImagesDst;
...
int HRP2IEEE1394ImagesInputMethod::GetImageSingleRaw(unsigned char **Image, int camera, struct timeval &timestamp)
{
  unsigned char * ImagesDst;
...
int HRP2IEEE1394ImagesInputMethod::GetImageRGB(unsigned char **ImageLeft, unsigned char **ImageRight, 
					       unsigned char **ImageUp, unsigned char **ImageWide)
{
  unsigned char * ImagesDst[4];
...
int HRP2IEEE1394ImagesInputMethod::SetImageSize(int lw, int lh, int CameraNumber)
{
  if (m_Board==0)
    return 0;
...
int HRP2IEEE1394ImagesInputMethod::GetImageSize(int &lw, int &lh, int CameraNumber)
{
...
string HRP2IEEE1394ImagesInputMethod::GetFormat()
{
  return m_Format;
}

int HRP2IEEE1394ImagesInputMethod::SetFormat(string aFormat)
{
  if (aFormat=="RGB")
    {
      m_Format = "RGB";
...
int HRP2IEEE1394ImagesInputMethod::SetParameter(string aParameter, string aValue)
{
...
void HRP2IEEE1394ImagesInputMethod::InitializeBoard()
{
  if (m_Board!=0)
    {
      bool lmComputing;
      lmComputing = m_Computing;
      m_Computing = 0;
      delete m_Board;
      m_Computing = lmComputing;
    }
  ...
  try 
    ...
    {
      ODEBUG("Exception during initialization: " << except.what() );
    }  
  catch(...)
    {
...
void HRP2IEEE1394ImagesInputMethod::StartContinuousShot()
{
  m_Board->continuousShot();
}

void HRP2IEEE1394ImagesInputMethod::StopContinuousShot()
{
  m_Board->stopContinuousShot();
}


void HRP2IEEE1394ImagesInputMethod::StopBoard()
{
  delete m_Board;
  m_Board = 0;
  ODEBUG("StopBoard");
...
int HRP2IEEE1394ImagesInputMethod::GetNumberOfCameras()
{
  if (m_Board==0)
    return 0 ;
...
void HRP2IEEE1394ImagesInputMethod::FromFrameRateToTime(int CameraNumber)
{
...
double HRP2IEEE1394ImagesInputMethod::NextTimeForGrabbing(int CameraNumber)
{
